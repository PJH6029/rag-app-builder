...
from rag.api import upload_data_to_s3, ingest_data, ingest_data_from_backup
...
def upload_to_s3():
    for root, dirs, files in os.walk(source_doc_root):
        for file in files:
            ...
            upload_data_to_s3(file_path, f"{source_doc_dir}/{object_location}")
...
def ingest_from_backup():
    cnt = ingest_data_from_backup("./backup")
    print(f"{cnt} parent chunks ingested")
